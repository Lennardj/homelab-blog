...
**A conditional `X-Forwarded-Proto` check does NOT fix this** — this was the first attempt and it failed. `ingress-nginx` defaults to `use-forwarded-headers: false`, so it *discards* the `X-Forwarded-Proto: https` that Cloudflare sends and rewrites the header from the actual connection scheme (`$scheme` = `http`). WordPress therefore never sees `https` in that header regardless of what Cloudflare sent, and the condition never fires. The working fix sets `$_SERVER['HTTPS'] = 'on'` unconditionally, which is correct because the only external path to this pod is Cloudflare → tunnel → ingress-nginx, always HTTPS from the client's side. Enabling `use-forwarded-headers` in the ingress-nginx ConfigMap would also work but is cluster-wide, affecting Grafana and Argo CD, so it was deliberately avoided. See Incident #24.
